...
class BANetFlowAugmentor(object):
    """对齐 BANet/banet-2d/core/utils/augmentor.py::FlowAugmentor 的增广。

    与官方一一对应：
      - color_transform: ColorJitter + AdjustGamma；非对称概率 ASYMMETRIC_PROB（官方 0.2）
      - eraser_transform: PROB（官方 0.5）；patch 数 randint(1, MAX_TIME+1)（官方 MAX_TIME=2）；
        patch 大小 randint(BOUNDS[0], BOUNDS[1])（官方 [50, 100]）；仅对右图擦除，颜色为右图均值
      - spatial_transform: 官方 spatial scale 段已注释 → 本实现也不缩放；
        DO_FLIP 默认 False（官方 train_stereo.py 也是 False）；
        YJITTER 默认 True（官方 `yjitter = not args.noyjitter`，--noyjitter 默认 False）。

    数据约定：sample['left'/'right'] 为 HxWx3 numpy；sample['disp'] 为 HxW numpy。
    """

    def __init__(self, config):
        self.crop_size = list(config.CROP_SIZE)
        self.do_flip = config.get('DO_FLIP', False)
        self.yjitter = config.get('YJITTER', True)

        brightness = config.get('BRIGHTNESS', 0.4)
        contrast = config.get('CONTRAST', 0.4)
        saturation_range = list(config.get('SATURATION_RANGE', [0.0, 1.4]))
        hue = config.get('HUE', 0.5 / 3.14)
        gamma = list(config.get('GAMMA', [1, 1, 1, 1]))

        self.asymmetric_color_aug_prob = config.get('ASYMMETRIC_PROB', 0.2)
        self.eraser_aug_prob = config.get('ERASER_PROB', 0.5)
        self.eraser_bounds = list(config.get('ERASER_BOUNDS', [50, 100]))
        self.eraser_max_time = config.get('ERASER_MAX_TIME', 2)

        self.h_flip_prob = 0.5
        self.v_flip_prob = 0.1

        self.color_jitter = ColorJitter(brightness=brightness, contrast=contrast,
                                        saturation=saturation_range, hue=hue)
        self.gamma_min, self.gamma_max = gamma[0], gamma[1]
        self.gain_min, self.gain_max = (gamma[2], gamma[3]) if len(gamma) >= 4 else (1.0, 1.0)

    def _photo(self, pil_img):
        out = self.color_jitter(pil_img)
        gain = random.uniform(self.gain_min, self.gain_max)
        gamma = random.uniform(self.gamma_min, self.gamma_max)
        # AdjustGamma 在 gamma=1, gain=1 时为 identity；保持与官方一致
        out = adjust_gamma(out, gamma, gain)
        return out

    def color_transform(self, img1, img2):
        if np.random.rand() < self.asymmetric_color_aug_prob:
            img1 = np.array(self._photo(Image.fromarray(img1)), dtype=np.uint8)
            img2 = np.array(self._photo(Image.fromarray(img2)), dtype=np.uint8)
        else:
            image_stack = np.concatenate([img1, img2], axis=0)
            image_stack = np.array(self._photo(Image.fromarray(image_stack)), dtype=np.uint8)
            img1, img2 = np.split(image_stack, 2, axis=0)
        return img1, img2
    ...
    def eraser_transform(self, img1, img2):
        ...
        if np.random.rand() < self.eraser_aug_prob:
            mean_color = np.mean(img2.reshape(-1, 3), axis=0)
            for _ in range(np.random.randint(1, self.eraser_max_time + 1)):
                x0 = np.random.randint(0, wd)
                y0 = np.random.randint(0, ht)
    ...
    def spatial_transform(self, img1, img2, disp):
        ...
        # 与官方一致：yjitter 时左/右图按不同 y 切，模拟不完美校正
        if self.yjitter and (ht - cy - 2) > 2 and (wd - cx - 2) > 2:
            y0 = np.random.randint(2, ht - cy - 2)
            x0 = np.random.randint(2, wd - cx - 2)
            y1 = y0 + np.random.randint(-2, 2 + 1)
            img1 = img1[y0:y0 + cy, x0:x0 + cx]
            img2 = img2[y1:y1 + cy, x0:x0 + cx]
            disp = disp[y0:y0 + cy, x0:x0 + cx]
        else:
            y0 = np.random.randint(0, max(1, ht - cy))
            x0 = np.random.randint(0, max(1, wd - cx))
            img1 = img1[y0:y0 + cy, x0:x0 + cx]
    ...
    def __call__(self, sample):
        ...
        img1 = sample['left'].astype(np.uint8)
        img2 = sample['right'].astype(np.uint8)
        disp = sample['disp']
