...
def _collect_kitti_split_paths(data_split) -> str:
    """拼接 TRAINING / EVALUATING / TESTING 路径字符串，用于判断 KITTI15 数据根。"""
...
def parse_config():
    ...
    for each in cfgs.DATA_CONFIG.DATA_INFOS:
        ...
        if dataset_name == 'KittiDataset':
            split_ref = _collect_kitti_split_paths(each.DATA_SPLIT)
            path_ref = split_ref + (str(yaml_dp) if yaml_dp else "")
            use_kitti15 = ("kitti15" in path_ref) or ("foundation_out" in path_ref)
            dataset_name = "KittiDataset15" if use_kitti15 else "KittiDataset12"
        if yaml_dp:
